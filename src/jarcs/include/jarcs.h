...
class ComputeShader
{
  public:
    ...
    RID create_shared_image_uniform(RID rid, const Ref<RDTextureView> &view, const int binding, const int set = 0);
    PackedByteArray get_image_uniform_buffer(RID rid, const int layer = 0) const;
    // RID create_texture2d_uniform(const Ref<ImageTexture>& image, Ref<RDTextureFormat>& format, const int binding,
    // const int set = 0); RID create_texture2d_sampler_uniform(const Ref<Image>& image, Ref<RDTextureFormat>& format,
    // const int binding, const int set = 0);
